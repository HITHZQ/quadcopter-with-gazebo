...
# 简易平面圆形轨迹生成 -> 简易平面八字形轨迹生成
def simple_trajectory(time):
    wait_time = 1.5     # 起飞到开始点等待时间
    height = 0.3        # 轨迹高度
    radius = 0.5        # 轨迹大小 (半长轴)
    # 频率参数
    freq_x = 0.2        # X轴频率 (例如 0.2 Hz)
    freq_y = 0.4        # Y轴频率 (X轴频率的两倍，确保是八字形)
    
    T = time - wait_time
    
    # 构建机头朝向 (保持指向切线方向，但为简化，我们先保持指向前方或切线)
    if T < 0:
        # 等待阶段：保持在起始点
        ...
        # x_dot = radius * (2*np.pi*freq_x) * np.cos(2*np.pi*freq_x * T)
        # y_dot = radius * (2*np.pi*freq_y) * np.cos(2*np.pi*freq_y * T)
        
        # 简单处理：保持机头朝向X轴，或者使用近似的切线方向
        # 我们使用切线方向作为heading
        # 为避免复杂，此处使用一个简单的前向向量，或者更精确地计算速度方向
        
        # 简化版：使用数值微分来估算切线方向 (速度方向)
        # 只需要一个微小的时间步长来计算下一个位置
        dT = 0.001 # 仿真步长
